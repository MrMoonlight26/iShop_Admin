...
export default async function ProductDetail({ params }: any) {
  ...
  // Prefer calling external API when API_BASE configured; otherwise route via internal proxy so HttpOnly cookies are used
  let url: string
  if (API_BASE) url = getApiUrl(`/api/v1/admin/catalog/${id}`)
  else url = `/api/proxy/admin/catalog/${id}`
