...
export default function ShopsPage() {
  ...
  async function fetchList() {
    try {
      setLoading(true)
      setError(null)
      const r = await api.get('/admin/shops', { params: { page: pageNumber, size: pageSize, ...(query && { q: query }), ...(statusFilter && { status: statusFilter }), ...(ownerTypeFilter && { ownerType: ownerTypeFilter }) } })
      const raw = r.data
  ...
  function openStatusModal(shop: any, target: string) {
  ...
  async function submitStatusChange() {
    ...
    try {
      const r = await api.patch(`/admin/shops/${shopId}/status`, { shopId, status: target, reason: statusModalReason })
  ...
  async function setApproval(id: string, approval: string) {
    try {
      const r = await api.patch(`/admin/shops/${id}/approval`, { shopId: id, approval })
      const updated = r.data
